...
qualenc_t* qualenc_alloc(quip_writer_t writer, void* writer_data)
{
    ...
    for (i = 0; i < read_pos_bins * qual_size * qual_size; ++i) {
        E->cs[i] = dist_alloc_encode(qual_size);
    }
...
void qualenc_encode(qualenc_t* E, const seq_t* x)
{
    ...
    /* case: i = 0 */
    if (n >= 1) {
        qualenc_encode_qk(E, 0 / bin_size,
                0, 0, qs[0]);
    }

    /* case: i = 1 */
    if (n >= 2) {
        qualenc_encode_qk(E, 1 / bin_size,
                0, qs[0], qs[1]);
    }

    /* case: i = 2 */
    if (n >= 3) {
        qualenc_encode_qk(E, 2 / bin_size,
                qs[0], qs[1], qs[2]);
    }

    /* case: i = 3 */
    if (n >= 4) {
        qualenc_encode_qk(E, 3 / bin_size,
                charmax2(qs[0], qs[1]),
                qs[2], qs[3]);
    }
    ...
    for (i = 4; i < n; ++i) {
        ...
        qualenc_encode_qk(E, i / bin_size, q2, q1, q);
    }
}
