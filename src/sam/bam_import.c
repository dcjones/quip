...
int sam_read1(tamFile fp, bam_header_t *header, bam1_t *b)
{
	...
	{ // seq and qual
		...
		z += str->l + 1;
		if (strcmp(str->s, "*") && c->l_qseq != (int32_t) strlen(str->s))
...
tamFile sam_open_in(quip_reader_t reader, void* reader_data)
{
	tamFile fp;
	fp = (tamFile)calloc(1, sizeof(struct __tamFile_t));

	fp->io.in.reader      = reader;
	fp->io.in.reader_data = reader_data;

	fp->str = (kstring_t*)calloc(1, sizeof(kstring_t));
	fp->ks = ks_init(&fp->io.in);
	return fp;
}
